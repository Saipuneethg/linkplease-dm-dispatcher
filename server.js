    ...
    try {
      ...
      try {
        ...
        if (response.status === 202 || response.status === 200) {
          const dmId = response.data && response.data.dm_id;
          job.dm_id = dmId || ('dm_' + Date.now());
          job.status = 'pending_check'; // Pending asynchronous delivery status reconciliation
          job.updatedAt = new Date();
          await job.save();
          apiSuccess = true;
        }
        ...
        if (status === 429) {
          ...
          job.status = 'queued'; // Requeue for next attempt
          job.last_error = '429 Too Many Requests';
          job.updatedAt = new Date();
          await job.save();
        } else if (status === 400) {
          // Fatal client error: Mark failed immediately
          job.status = 'failed';
          job.last_error = '400 Fatal Bad Request';
          job.updatedAt = new Date();
          await job.save();
          apiSuccess = true;
        } else {
          ...
          job.send_attempts += 1;
          job.last_error = err.message || '500 Internal Server Error';
          job.updatedAt = new Date();

          if (job.send_attempts >= 5) {
            job.status = 'failed';
          } else {
            job.status = 'queued';
            const backoff = Math.pow(2, job.send_attempts) * 1000;
      ...
      for (const job of pendingJobs) {
        try {
          ...
          if (currentStatus === 'delivered' || currentStatus === 'sent') {
            ...
            job.reconciliation_retries += 1;
            job.updatedAt = new Date();

            if (job.reconciliation_retries < 3) {
              // Retry sending up to 3 times before setting final failed status
              job.status = 'queued';
              job.dm_id = null;
            } else {
              job.status = 'failed';
